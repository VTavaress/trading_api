...
from modules.backtests import services
...
def run_backtest_endpoint(request: BacktestRequest, db: Session = Depends(get_db)):
    try:
        backtest_run = services.run_backtest(
            db=db, ticker=request.ticker, start_date=request.start_date,
            end_date=request.end_date, strategy_type=request.strategy_type,
            strategy_params=request.strategy_params, initial_cash=request.initial_cash,
            commission=request.commission
        )
        return {
            "id": backtest_run.id, "status": backtest_run.status,
            "ticker": backtest_run.ticker, "strategy_type": backtest_run.strategy_type,
